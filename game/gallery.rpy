...
init python:
    ...
    cantidad_paginas_cg=math.ceil(len(list_gal_cg)/float(4))
    
    list_gal_bonus=["thanks1year"]
    list_gal_bonus_img=["/gui/gallery/BONUS/thanks.png"]
    cantidad_paginas_bonus=math.ceil(len(list_gal_bonus)/float(4))
    
    def init_gallery(gal,list_gal,type_str=""):
      for i in list_gal:
        aux=i
        gal.button(i)
        gal.condition("persistent."+type_str+aux.replace(" ","_"))
        gal.image(i)
...
screen gallery():
    ...
    hbox:
        ...
        text "{font=fonts/PleasantlyPlump-pRv1.ttf}{color=#7f7ab1}Page [page_gallery]{/color}{/font}" xalign 0.5 yalign 0.5
